...
{
Tracker::Tracker(const Eigen::MatrixXd& Q, const Eigen::MatrixXd& R, double max_match_distance, int tracking_threshold,
                 int lost_threshold)
  : tracker_state(LOST)
  , tracking_id(0)
  , Q_(Q)
...
void Tracker::init(const Armors::SharedPtr& armors_msg)
{
  ...
  // KF init
  kf_ = std::make_unique<KalmanFilter>(6, 3);
  kf_->Q = Q_;
  kf_->R = R_;
...
void Tracker::update(const Armors::SharedPtr& armors_msg, const double& dt)
{
  ...
  Eigen::VectorXd kf_prediction = kf_->predict(Eigen::VectorXd::Zero(3));
  ...
  if (!armors_msg->armors.empty())
  {
    ...
    if (min_position_diff < max_match_distance_)
    {
      ...
      matched = true;
      Eigen::Vector3d position_vec(matched_armor.position.x, matched_armor.position.y, matched_armor.position.z);
