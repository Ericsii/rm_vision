  ...
  : Node("projectile_motion_node", options.use_intra_process_comms(true))
{
  ...
  gimbal_cmd_publisher_ = this->create_publisher<rm_interfaces::msg::GimbalCmd>(gimbal_cmd_topic_, 10);
  shoot_data_subscriber_ = this->create_subscription<rm_interfaces::msg::RobotShootData>(
      shoot_data_topic_, 10, std::bind(&ProjectileMotionNode::shoot_data_callback, this, std::placeholders::_1));
  ...
  tf_filter_ = std::make_shared<tf2_filter>(target_subscriber_, *tf_buffer_, target_frame_, 10,
...
void ProjectileMotionNode::target_callback(const auto_aim_interfaces::msg::Target::SharedPtr msg)
{
  ...
  try
  {
    transform_stamped = tf_buffer_->lookupTransform(target_frame_, msg->header.frame_id, target_time);
  }
  ...
  Eigen::Quaterniond q_shooter_link =
      Eigen::Quaterniond(transform_stamped.transform.rotation.w, transform_stamped.transform.rotation.x,
...
void ProjectileMotionNode::shoot_data_callback(const rm_interfaces::msg::RobotShootData::SharedPtr msg)
{
